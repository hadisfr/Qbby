...
{
	SDL_Surface* res = find_in_cache(filename);
	if (res == NULL){
		res = SDL_LoadBMP(filename.c_str());
		bmp_cache.push_back(new ImageLoad(filename, res));
	}
...
SDL_Surface* window::find_in_cache(string address){
	for(int i=0; i<bmp_cache.size(); i++){
		if(bmp_cache[i]->address == address)
			return bmp_cache[i]->sdl_surface;
	}
	return NULL;
}
